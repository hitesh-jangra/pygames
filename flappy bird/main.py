...
def welcomeScreen():
	...
	while True:
		for event in pygame.event.get():
			#if user click on cross butoon, cose the game
			if event.type==QUIT or (event.type==KEYDOWN and event.key == K_ESCAPE):
				pygame.quit()
				sys.exit()

			#if user presses space key or up key
...
def mainGame():
	...
	while True:
		for event in pygame.event.get():
			if event.type==QUIT or(event.type==KEYDOWN and event.type==K_ESCAPESC):
				pygame.quit()
				sys.exit()
		...
		crashTest=isCollide(playerx,playery,upperPipes,lowerPipes) #function will return true if you are crashed
		if crashTest:
			return
...
def getRandomPipe():
	...
	#one straight and one rotated(180)
	pipeHeight=GAME_SPRITES['pipe'][0].get_height()
...
if __name__ == '__main__':   
	...
	while True:
		welcomeScreen()   #shows welcome screen to user until press cross button
